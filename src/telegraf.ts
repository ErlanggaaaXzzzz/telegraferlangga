...
function defaultErrorLogger<C extends Context>(err: unknown, ctx: C) {
  const updateId = ctx.update.update_id
  const chatId = ctx.chat?.id
  const message = err instanceof Error ? err.message : String(err)
  console.error(
    `[telegraf-erlangga] error on update ${updateId}${
...
function printLaunchBanner(
  ...
  console.log(' telegraf-erlangga')
  console.log(` bot        : @${botInfo.username}`)
  console.log(` mode       : ${mode}${webhookUrl ? ` (${webhookUrl})` : ''}`)
...
function validateBotToken(token: string): void {
  if (typeof token !== 'string' || token.trim().length === 0) {
    throw new Error(
      '[telegraf-erlangga] Bot token is missing. Get one from @BotFather and pass it to `new Telegraf(token)`.'
    )
  }
  if (!BOT_TOKEN_PATTERN.test(token)) {
    throw new Error(
      '[telegraf-erlangga] Bot token format looks invalid. Expected something like "123456789:AAbecomeTheTokenFromBotFather". Double check you copied the full token from @BotFather.'
    )
  }
}
...
export class Telegraf<C extends Context = Context> extends Composer<C> {
  ...
  private errorLogger?: (err: unknown, ctx: C) => void
  ...
  constructor(token: string, options?: Partial<Telegraf.Options<C>>) {
    ...
    debug('[telegraf-erlangga] Created a `Telegraf` instance')
  }
  ...
  /**
   * Registers a console/custom logger that runs on every unhandled error,
   * without replacing the default error behavior (still sets exit code and rethrows
   * unless overridden separately via `.catch()`).
   */
  logErrors(logger: (err: unknown, ctx: C) => void = defaultErrorLogger) {
    this.errorLogger = logger
    return this
  }

  /**
   * Sets the bot owner id(s), used by `ctx.isOwner()` (no-args form),
   * `commandOwner`, and other owner-gated helpers.
   */
  owner(ids: number | number[]) {
    this.ownerIds = Array.isArray(ids) ? ids : [ids]
    return this
  }

  /**
   * Registers a command that only runs if the sender's id is in the
   * configured owner list (set via `bot.owner(...)`). Silently ignores
   * the command for everyone else.
   */
  commandOwner(command: string | string[], ...fns: Middleware<C>[]) {
    return this.command(command, async (ctx, next) => {
      if (!this.ownerIds.includes(ctx.from?.id as number)) return
      for (const fn of fns) await Composer.unwrap(fn)(ctx as unknown as C, next)
    })
  }

  /**
   * Registers a command that only runs if the sender is an admin/creator
   * of the current chat (always allowed in private chats). Silently
   * ignores the command otherwise.
   */
  commandAdmin(command: string | string[], ...fns: Middleware<C>[]) {
    return this.command(command, async (ctx, next) => {
  ...
  /**
   * Sends a text message to multiple chat ids sequentially, automatically
   * handling Telegram flood-wait (429) errors between sends.
   * @see Telegram.broadcastMessage
   */
  broadcast(
    chatIds: (number | string)[],
    text: Parameters<Telegram['sendMessage']>[1],
    extra?: Parameters<Telegram['sendMessage']>[2]
  ) {
    return this.telegram.broadcastMessage(chatIds, text, extra)
  }
  ...
  async launch(
    ...
    if (webhook === undefined) {
      ...
      printLaunchBanner(this.botInfo, 'polling')
      return
    }
    ...
    printLaunchBanner(this.botInfo, 'webhook', domainOpts.url)
  }
  ...
  async handleUpdate(update: tg.Update, webhookResponse?: http.ServerResponse) {
    ...
    if (this.ownerIds.length > 0) ctx.state.ownerIds = this.ownerIds
    try {
      await pTimeout(
