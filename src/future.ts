...
const replyContext: ReplyContext = {
  ...
  async replySafe(this: Context, text, extra) {
    try {
      return await replyContext.reply.call(this, text, extra)
    } catch (err) {
      return undefined
    }
  },
  replyHTML(this: Context, html, extra) {
    return replyContext.replyWithHTML.call(this, html, extra)
  },
  replyMD(this: Context, markdown, extra) {
    return replyContext.replyWithMarkdownV2.call(this, markdown, extra)
  },
  async replyWithAutoDelete(this: Context, text, seconds, extra) {
    const sent = await replyContext.reply.call(this, text, extra)
    setTimeout(() => {
      this.telegram.deleteMessage(sent.chat.id, sent.message_id).catch(() => {})
    }, seconds * 1000)
    return sent
  },
  async replyLong(this: Context, text, extra, chunkSize = 4096) {
    ...
    for (const chunk of chunks) {
      sentMessages.push(await replyContext.reply.call(this, chunk, extra))
    }
    return sentMessages
  },
...
export function useNewReplies<C extends Context>(): Middleware<C> {
  return (ctx, next) => {
    ...
    ctx.replySafe = replyContext.replySafe
    ctx.replyHTML = replyContext.replyHTML
    ctx.replyMD = replyContext.replyMD
    ctx.replyWithAutoDelete = replyContext.replyWithAutoDelete
    ctx.replyLong = replyContext.replyLong
    return next()
  }
}
