...
const debug = d('telegraf:telegram')
...
export class Telegram extends ApiClient {
  ...
  /**
   * Same as {@link Telegram.sendMessage}, but automatically retries once
   * if Telegram responds with a 429 (flood wait), waiting the amount of
   * time Telegram asks for via `retry_after`.
   * @param maxRetries Maximum number of retry attempts on 429 errors (default 1)
   */
  async sendMessageSafe(
    ...
    for (;;) {
      try {
        return await this.sendMessage(chatId, text, extra)
      } catch (err) {
        if (
          ...
          debug(
            'sendMessageSafe hit flood wait, retrying after %ds (attempt %d/%d)',
            retryAfter,
            attempt + 1,
            maxRetries
          )
  ...
  /**
   * Sends the same text message to multiple chats sequentially,
   * automatically handling flood-wait (429) errors between sends.
   * Returns a summary of which chat ids succeeded and which failed.
   * @param chatIds List of chat ids or usernames to send the message to
   * @param delayMs Delay in milliseconds between each send (default 50ms)
   */
  async broadcastMessage(
    ...
    const succeeded: (number | string)[] = []
    const failed: { chatId: number | string; error: unknown }[] = []

    for (const chatId of chatIds) {
      try {
        await this.sendMessageSafe(chatId, text, extra)
        succeeded.push(chatId)
      } catch (err) {
        debug('broadcastMessage failed for chat %s', chatId, err)
        failed.push({ chatId, error: err })
      }
      if (delayMs > 0) {
