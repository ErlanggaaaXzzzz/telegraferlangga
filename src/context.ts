...
export class Context<U extends Deunionize<tg.Update> = tg.Update> {
  ...
  async replySafe(...args: Shorthand<'sendMessage'>) {
    try {
      return await this.reply(...args)
    } catch (err) {
      debug('replySafe caught error', err)
      return undefined
    }
  }

  replyHTML(html: string, extra?: tt.ExtraReplyMessage) {
    return this.replyWithHTML(html, extra)
  }

  replyMD(markdown: string, extra?: tt.ExtraReplyMessage) {
    return this.replyWithMarkdownV2(markdown, extra)
  }

  async replyWithAutoDelete(
    text: string,
    seconds: number,
    extra?: Shorthand<'sendMessage'>[1]
  ) {
    const sent = await this.reply(text, extra)
    setTimeout(() => {
      this.telegram.deleteMessage(sent.chat.id, sent.message_id).catch((err) => {
        debug('replyWithAutoDelete failed to delete message', err)
      })
    }, seconds * 1000)
    return sent
  }
  ...
  async replyLong(
    ...
    if (text.length <= chunkSize) return [await this.reply(text, extra)]
    ...
    for (const chunk of chunks) {
      sentMessages.push(await this.reply(chunk, extra))
    }
    return sentMessages
  }

  isOwner(ownerIds?: number | number[]) {
    const ids = ownerIds ?? (this.state.ownerIds as number[] | undefined)
    if (ids === undefined) return false
    const list = Array.isArray(ids) ? ids : [ids]
    const userId = this.from?.id
    return userId !== undefined && list.includes(userId)
  }

  async isAdmin() {
    this.assert(this.chat, 'isAdmin')
    const userId = this.from?.id
    if (userId === undefined) return false
    if (this.chat.type === 'private') return true

    const member = await this.telegram.getChatMember(this.chat.id, userId)
