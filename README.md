> **telegraf-erlangga** is a fork of [Telegraf](https://github.com/telegraf/telegraf), extended with extra `ctx` helper methods for everyday bot development. It is not affiliated with or endorsed by the original Telegraf maintainers. Full credit to the original authors — see [LICENSE](./LICENSE).
>
> ## Extra helpers added in this fork
>
> - `ctx.replySafe(...)` — same as `ctx.reply`, but never throws; catches send errors and returns `undefined` instead of crashing your bot.
> - `ctx.replyHTML(html, extra?)` — shorthand for `ctx.replyWithHTML`.
> - `ctx.replyMD(markdown, extra?)` — shorthand for `ctx.replyWithMarkdownV2`.
> - `ctx.replyWithAutoDelete(text, seconds, extra?)` — sends a message and deletes it automatically after `seconds`.
> - `ctx.replyLong(text, extra?, chunkSize?)` — automatically splits messages longer than 4096 characters (Telegram's limit) into multiple sends.
> - `ctx.isOwner(ownerIds)` — checks whether the sender's user id matches a given id or list of ids.
> - `ctx.isAdmin()` — async check for whether the sender is an admin/creator of the current chat (`true` in private chats).
> - `Markup.pagination({ page, totalPages, ... })` — builds a Prev/Next inline keyboard for paginated lists.
> - `Markup.confirm(yesData, noData, options?)` — builds a Yes/No inline keyboard in one call.
> - `Markup.menuFromObject({ Label: 'callback_data' })` — builds an inline keyboard from a plain object.
> - `Markup.urlButtonRow({ Label: 'https://...' })` — builds a single row of URL buttons from a plain object.
> - `Markup.toggleButton(label, isActive, callbackData)` — builds a single button with a ✅/❌ prefix for on/off settings.
> - `telegram.sendMessageSafe(chatId, text, extra?, maxRetries?)` — like `sendMessage`, but auto-retries on Telegram 429 flood-wait using `retry_after`.
> - `telegram.broadcastMessage(chatIds, text, extra?, delayMs?)` — sends a message to many chat ids sequentially with flood-wait handling; returns `{ succeeded, failed }`.
> - `bot.owner(id | id[])` — sets the bot's owner id(s), enabling `ctx.isOwner()` to be called with no arguments.
> - `bot.commandOwner(command, ...handlers)` — registers a command that silently no-ops unless the sender is a configured owner.
> - `bot.commandAdmin(command, ...handlers)` — registers a command that silently no-ops unless the sender is a chat admin/creator (always allowed in private chats).
> - `bot.broadcast(chatIds, text, extra?)` — shorthand for `telegram.broadcastMessage`.
> - `bot.logErrors(logger?)` — attaches a logger that runs on every unhandled error, without replacing default error behavior; defaults to a built-in console logger if no argument is given.
> - `new Telegraf(token)` — validates the token format up front and throws a clear error immediately if it's missing or malformed, instead of failing later on the first API call.
> - `bot.launch(...)` — prints a startup banner (bot username, mode, start time) to the console on successful launch, instead of Telegraf's silent debug-only log.
>
> Everything else below is the original Telegraf documentation and still applies as-is.
